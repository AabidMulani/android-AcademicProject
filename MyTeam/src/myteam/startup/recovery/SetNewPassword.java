...
public class SetNewPassword extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(value==null){
			Toast.makeText(this, "UNEXPEXTED ERROR...!", Toast.LENGTH_SHORT).show();
		}
		...
		Submit.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				}else if(!Pass.getText().toString().equals(PassRe.getText().toString())){
					ShowThisMsg("Error:	", "Password and Re-Entered Password do NOT match", false);
				}else{
					StoreNewPassword();
				}
			}
	...
	private void StoreNewPassword() {
		...
		new PostToServer(nameValuePairs,URL_Strings.getUrlForgetPassword(),"Updating Password..").execute();
	}
	
	
	protected void ShowThisMsg(String title, String message, final boolean quit) {
		DIALOG_SHOW_MSG = new AlertDialog.Builder(SetNewPassword.this)
...
public PostToServer(List<NameValuePair> nameValuePairs,String URL,String Msg) {
		...
		protected void onPreExecute() {
			super.onPreExecute();
			RESULT_STRING = null;
			PG = new ProgressDialog(SetNewPassword.this);
		...
		protected void onPostExecute(Boolean result) {
			...
			if(result){
				if(RESULT_STRING.equalsIgnoreCase("Done")){
					ShowThisMsg("Successfull", "NEW PASSWORD HAS BEEN SET..", true);
				}else{
